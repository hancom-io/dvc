...
namespace CheckListModule {
    class CHyperlink : public IHyperlink
    {
    public:
        CHyperlink(std::string dvcFile);
        CHyperlink(LPCSTR document);
        ~CHyperlink();

    public:
        DVCType getType() { return DVCType::Hyperlink; }
        std::map<std::string, int>    getMapData() { return m_mapCheckedList; }
        std::vector<UINT>        getCheckedList() { return m_checkedList; }
                
        bool                    getPermission() { return m_permission; }
