...
namespace CheckListModule {
    class CParaNumBullet : public IParaNumBullet
    {
    public:
        CParaNumBullet(std::string dvcFile);
        CParaNumBullet(LPCSTR document);
        ~CParaNumBullet();

    public:
        DVCType getType() { return DVCType::ParaNumBullet; }
        std::map<std::string, int>    getMapData() { return m_mapCheckedList; }
        std::vector<UINT>            getCheckedList() { return m_checkedList; }

        ParaNumType                    getParaNumType();
        bool                        getStartNumber();
        UINT                        getValue();
        std::vector<LevelType*>        getLevelTypes();
    ...
    private:
        ...
        ParaNumType                    m_paraNumType;
        bool                        m_startNumber;
        UINT                        m_value;
        std::vector<LevelType*>        m_levelTypes;
    };
}
