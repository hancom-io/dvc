...
static std::map<std::string, int> macroMapData = {
    {JIN_MACRO,                                JID_MACRO},
    {JIN_MACRO_PERMISSION,                    JID_MACRO_PERMISSION}
};

namespace CheckListModule {

    CMacro::CMacro(std::string dvcFile)
    {
        m_dvcFile = dvcFile;
        parsingDVCFile(m_dvcFile);
    }

    CMacro::CMacro(LPCSTR document)
    {
    ...
    bool CMacro::getPermission()
    {
        return m_permission;
    }

    bool CMacro::parsingDVCFile(std::string dvcFile)
    {
        bool bResult = false;
        ...
        {
            Json::Value value = root[JIN_MACRO];
    ...
    bool CMacro::parsingDVCDocument(LPCSTR document)
    {
        bool bResult = false;
    ...
    bool CMacro::parsingElement(Json::Value root, void* Obj)
    {
        ...
        while (it != root.end())
        {
            ...
            mapIter = macroMapData.find(it.name().c_str());    // 또는 it.key()로 해보자.
